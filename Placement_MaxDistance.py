    ...
    for i in range(len(listCC)):
        ...
        if len(component.edges()) !=0:
            for i in component.edges():
                Link_to_add.append((min(i[0],i[1]),max(i[0],i[1])))
    for i in range(len(minD_node)-1):
        Link_to_add.append((min(minD_node[i],minD_node[i+1]),max(minD_node[i],minD_node[i+1])))
    ...
    while change==1:
        ...
        for node in range(n):
            if degree[node]==1:  # degree=1
                ...
                change=1
    # for key,value in Adj_dic2.items():
    #     print(Adj_dic1[key],Adj_dic2[key])
...
def R(G,SensorPlacement,probability):
    g=G.copy()
    n=g.number_of_nodes()
    Link_to_add=Link_between_sensor(SensorPlacement,g)
    for edge in Link_to_add:
...
for K in [2]:
    R_result=[]
    PlaceKSensor=MaxDistance_placement(G,K)
    print('PlaceKSensor: ',PlaceKSensor)
    print(R(G,PlaceKSensor,probability))