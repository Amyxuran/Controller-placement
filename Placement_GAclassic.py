...
def initial_population(G,K,p,NodeSet):
    ...
    # group_num=max((2,np.ceil(n/100*np.log(S)/d)))
    group_num=2
...
def select_cross_mutate(G,p,K,best_R,worst_R,dic1,dic2,NodeSet,crossover_rate,mutation_rate,elitism_num):
    NodeSet=list(NodeSet)
    population_size=len(dic1)-elitism_num
    remain_placement=[]
    remain_R=[]
    a=sorted(dic2.items(),key=lambda x:x[1],reverse=True)
    for i in range(elitism_num):
        remain_placement.append(dic1[a[i][0]])
        remain_R.append(a[i][1])
    
    # select: tournament selection
    pop1={}
    pop2={}
    for i in range(population_size):
        a,b=np.random.choice(range(population_size),2,False)
        choice1,choice2=dic1[a],dic1[b]
        choice1_R,choice2_R=dic2[a],dic2[b]
        if choice1_R>=choice2_R:
            pop1[i]=choice1
            pop2[i]=choice1_R
        else:
            pop1[i]=choice2
    ...
    # cross: single point crossover
    new_pop1={}
    new_pop2={}
    cross_pairs_num=round(population_size*crossover_rate/2)*2
    k=0
    for i in range(0,population_size,2):
        if i <=cross_pairs_num:
            parent1=pop1[i]
            parent2=pop1[i+1]
            # random.shuffle(parent1)
            # random.shuffle(parent2)
            cross_point=np.random.randint(1,K)
            child1_temp=parent1.copy()
            child2_temp=parent2.copy()
            child1_temp[cross_point:]=parent2[cross_point:].copy()
            child2_temp[cross_point:]=parent1[cross_point:].copy()
            ## repeat detect
            a=child1_temp[:cross_point]
            b=child1_temp[cross_point:]
            child1=b.copy()
            c=child2_temp[:cross_point]
            d=child2_temp[cross_point:]
            child2=d.copy()
            for i in a:
                while i in b:
                    i = d[b.index(i)]
        ...
        if np.random.rand()< mutation_rate:
            while True:
                ...
                b=np.random.randint(0,len(NodeSet))
                if NodeSet[b] not in child1:
                    child1[a]=NodeSet[b]
                    break
        if np.random.rand()< mutation_rate:
            while True:
                a=np.random.randint(0,K)
                b=np.random.randint(0,len(NodeSet))
                if NodeSet[b] not in child2:
                    child2[a]=NodeSet[b]
                    break
        
        new_pop1[k]=child1
        new_pop1[k+1]=child2
        new_pop2[k]=R(G,child1,p)
        new_pop2[k+1]=R(G,child2,p)
        k=k+2
    for i in range(elitism_num):
        new_pop1[k]=remain_placement[i]
        new_pop2[k]=remain_R[i]
        k=k+1
    
    best_R=max(new_pop2.items(),key=lambda x:x[1])[1]
    worst_R=min(new_pop2.items(),key=lambda x:x[1])[1]
    return best_R,worst_R,new_pop1,new_pop2
...
def main():
    ...
    for graphnum in Real_index:
        ...
        for K in [2,3,4,5,6,7,8,9,10]:
            ...
            best_R,worst_R,dic1,dic2=initial_population(G,K,probability,G.nodes())
            MaxIterations=int(np.ceil(1500/len(dic1)))
            for iteration in range(MaxIterations):
                best_R,worst_R,dic1,dic2=select_cross_mutate(G,probability,K,best_R,worst_R,dic1,dic2,G.nodes(),crossover_rate,mutation_rate,elitism_num)
            for key,value in dic2.items():
                if value==best_R:
                    R_result.append(dic2[key])
                    S_placement.append(sorted(dic1[key]))
                    break
        with open('R_result_Real_GA_classic1_0.99.txt', 'a', encoding='utf-8') as file1:
            for i in R_result:
                print(i,file=file1)
        with open('S_placement_Real_GA_classic1_0.99.txt', 'a', encoding='utf-8') as file1:
